...
% https://www.paypal.me/RodyO/3.5
    ...
    %split help args from varargin
 if ~isempty(find(strcmp(varargin,'helpfun')))   
    idxhelp=[find(strcmp(varargin,'helpfun')) find(strcmp(varargin,'helpfun'))+1];
    varhelp=varargin(idxhelp);
    varargin=varargin(setdiff(1:length(varargin),idxhelp));
 else
     idxhelp=[];
     %helparg=varargin{find(strcmp(varargin,'helpfun')+1}
...
edit_bgcolor = 'White';
    ...
    if ~isempty(idxhelp)
        pos1=[separator_offset_X,2, total_width/2.5,control_height*1.5];
        pos2=[total_width*(1-1/2.5)-separator_offset_X,2    total_width/2.5,control_height*1.5];
        addhelp(varhelp,[pos1;pos2]);
    end
    ...
    %% Helper funcitons
    function  addhelp(args,posbuttons)
     
        
      pos=...
      [posbuttons(1,1)+posbuttons(1,3) posbuttons(1,2) posbuttons(2,1)-(posbuttons(1,1)+posbuttons(1,3)) posbuttons(1,4)];
      if pos(3)<20
         pos(3)=20;
      end
         % Draw OK button
