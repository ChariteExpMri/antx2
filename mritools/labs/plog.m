...
% a..aligntment: a=1: alignment left (default), a=2:alignment right
...
if  exist('d')~=1       ;   dec=3          ; else; dec   =d                ;end %'decimalpoints'
if  exist('s')~=1       ;   spacer=2       ; else; spacer=s                ;end%'spacer'
if  exist('cr')~=1      ;   cariageReturn=1; else; cariageReturn=cr        ;end%'cariageReturn'
if  exist('align')~=1   ;   align=1        ; else; align=a                 ;end %'alignment'

if spacer<2; spacer=2; end
...
%     for j=1:M
%         if ischar(e2{i,j})
%             e2{i,j} =[   sprintf(['%#',num2str(ff-dec-1), 's '],e2{i,j}) ];
%         else
%             if round(e2{i,j})==e2{i,j}
%                 e2{i,j} =[   sprintf(['%#',num2str(ff-dec-1), 'd '],e2{i,j}) ];
%             else
%                 e2{i,j} =[   sprintf(['%#',num2str(ff),'.',num2str(dec),'f '],e2{i,j}) ];
%                 if dec==0
%                    e2{i,j}= regexprep(e2{i,j},'\.','') ;
%                 end
%             end
%         end
%     end
    ...
    for i=1:M;%add spacer and middleOrientation
        ...
        for j=1:N
            if length(e{j,i})~=si
                ...
                %  e{j,i}= [  e{j,i}  repmat(' ',[1  (si-length(e{j,i})  )])      ]; %LEFT ALLIGN
                           e{j,i}= [  repmat(' ',[1  (si-length(e{j,i})  )])   e{j,i}     ]; %RIGHT ALLIGN
            end
        end
    end
else
