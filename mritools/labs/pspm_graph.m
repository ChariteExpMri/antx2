function [Y,y,beta,Bcov] = pspm_graph(xSPM,SPM,hReg,Ic)
...
%         figure(Fgraph)
        ...
        figure(1122);clf,set(gcf,'color','w','units','normalized','tag','EOIbar');
        
       
        
        if isempty(fig)
        set(gcf,'position',[0.7174    0.6100    0.2799    0.2722]);
        end
        %subplot(2,1,2)
        cla
        hold on
...
%         end
        he=errorbar(1:length(cbeta),cbeta, CI,'k.'     );
        ...
        hold off;
        info.title=xSPM.title;
        setappdata(gca,'info', info );
...
% fig3=findobj(0,'tag','EOIbar');
% us=get(fig3,'userdata');
% us.title=xSPM.title;
% set(fig3,'userdata',us);
