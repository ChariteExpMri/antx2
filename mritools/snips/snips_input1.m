...
% pipeline
% MPM-pipeline-invivo
%% use an independent project for MPM, because native space is T1/PD/MT-space
%% i.e. turborare is registered to T1/PD/MT
%% ===================================================================
...
v.study      ='H:\Daten-2\Imaging\AG_Penack_ICANS\MPM_analysis'; %study-folder
v.paraw      ='H:\Daten-2\Imaging\AG_Penack_ICANS\raw' ; %Bruker-rawdata-folder
v.mpm        ='D:\MATLAB\mpm_rodent'                ;%PATH of: wrapper functions
...
xrename(0,['.*' 'TurboRARE'     '.*'],'in_t2.nii','copy'); 
dispfiles;% check files
 
%% =========================================================================================================================
%%  [5] change ANTx-config: 
%% no skullstripped brains, thus use default 'usePriorskullstrip'-method
%% ==========================================================================================================================
...
%% ========================================================================
%% [10] get orientation from 'in_t2.nii'-space to 'T1/MT/PD'-space
...
% --> here the best preorientation is: rotTable-Index [2]: which is '0 0 0'
%--> thus, in the next step we'll set 'mpm.t2w_preorient' to [0 0 0]
 
%% ================================================================
%% [11] set preorientation from 'in_t2.nii'-space to 'AVGT.nii'-space
%% via f_setup('update',...
%% ================================================================
f_setup('update','mpm.MPM_path',v.MPMtoolbox, 'mpm.t2w_preorient',[0 0 0]); 
...
%% please check HTML-file for best preorientation ..than go to next step
%% ============================================================================
antcb('copytemplates'); %get template
antcb('selectdirs','all');
mdirs= antcb('getsubjects');
...
%% best preorientation is rotTable-Index [1]:
%% ==========================================================================
antcb('set','wa.orientType',1);
...
%% #################################################
% pipeline
% DWI-analysis part-1 (MULTISHELL): Matlab-preprocessing
% - MULTISHELL-DWI-pipeline covers the following: 
%     make ANTx-project, Brukerimport, renaming files, transform to standardspace, complete DTIprep-routine
%% ******************************************************************************************
%% ******************************************************************************************
%%  PART-1: make project, Bruker import/rename files,register to standard space 
%% ******************************************************************************************
%% ******************************************************************************************
%% ===============================================
%% [0]  set parameter (paths of study and raw-data)
%% ===============================================
try; antcb('close'); end
cf;clear all, clc, warning off;
 
v.study      ='H:\Daten-2\Imaging\AG_Pruess\2025_NMDR_LGI1'; %study-folder
v.paraw      =fullfile(v.study,'raw') ; %Bruker-rawdata-folder
cd(v.study);
...
%%   [2] Bruker import
%% ===============================================
%% make list of all bruker files
w1=xbruker2nifti(paraw,0,[],[],'gui',0,'show',1); %get all Bruker-raw wiles

%% ===============================================
%% inspect and import Bruker DWI-files
w2=xbruker2nifti(w1,0,[],[],'gui',0,'show',1,'flt',... %show DWI-files
    {'protocol','DTI_EPI_seg.*dir$'});
w3=xbruker2nifti(w2,0,[],[],'gui',0,'show',0,...       %import files
  'ExpNo_File',0,'overwrite',0);

%% inspect and import Bruker tubrorare-files
w4=xbruker2nifti(w1,0,[],[],'gui',0,'show',1,'flt',{'protocol','T2_TurboRARE'});%show turonrate-files
w5=xbruker2nifti(w4,0,[],[],'gui',0,'show',0, 'ExpNo_File',0,'overwrite',0);%import files
antcb('update'); %update GUI
...
%% ===============================================
antcb('selectdirs','all'); %select all animals
xrename(0,'04_9_DTI_EPI_seg_b100_6dir.nii'       , 'inDWI_b100.nii',':');
xrename(0,'04_3_DTI_EPI_seg_b900_13dir.nii'      , 'inDWI_b900.nii',':');
xrename(0,'04_5_DTI_EPI_seg_b1600_25dir.nii'     , 'inDWI_b1600.nii',':');
xrename(0,'04_7_DTI_EPI_seg_b2500_37dir.nii'     , 'inDWI_b2500.nii',':');
xrename(0,'02_T2_TurboRARE_MapShim.nii'          , 't2.nii',':');
dispfiles; %count/show fileNames over all animals
...
%% ==============================================
%%   [5] copy the here used DWI-atlas to this study
%% ===============================================
atlassource='D:\MATLAB\atlases\DTI_atlas_31mar20';  %path of used DWI/DTI-atlas
[~,atlasname]=fileparts(atlassource);
v.DWIatlas=fullfile(v.study,atlasname);
copyfile(atlassource,v.DWIatlas,'f');

%% =========================================================================
%%  [6] DTIprep-setup  : here we use Single-shell-approach!
%% ==========================================================================
DTIprep('delete'); %delete existing "DTI"-folder
clear d;
[rawdirs]  = spm_select('FPList',v.paraw,'dir'); rawdirs=cellstr(rawdirs); % get raw-dirs
dwifileTag=['^inDWI_.*.nii']; % idential input-string of DWIfiles
[dwifiles] = spm_select('FPListRec',fullfile(v.study,'dat'),dwifileTag);
dwifiles=cellstr(dwifiles); %get DWI-epi-files
[~,dwifilesShort]=fileparts2(dwifiles); 
dwifilesShort=unique(dwifilesShort);
dwifilesShort=natsort(dwifilesShort); %SORT DWI-files in correct order

d.brukerdata     =rawdirs{1} ; % % sample Bruker-rawdata set from one animal to obtain the b-table(s)
d.DTItemplate    =spm_select('FPList',v.DWIatlas,'^DTI.*.nii') ; % % DTI-atlas NIFTI-file
d.DTItemplateLUT =spm_select('FPList',v.DWIatlas,'^DTI.*.txt') ; % % DTI-atlas txt-file with labels
d.DTIfileName    =dwifilesShort ; % % filenames of the DWI/DTI-files located in animal folders
DTIprep('ini',d);  % % initialize DTIprep-struct and fill requested parameters
DTIprep('import'); % % import b-table and DTItemplate/DTItemplateLU

%% ==============================================
%%  [7] run DTIprep-PREPROCESSING
%% ===============================================
DTIprep; % DTI-open GUI
antcb('selectdirs','all');   % select all animal-folders
DTIprep('run','all');
...
%% ======================================================================
%%   [8] copy the latest script that is used to export the data to HPC
%% note: the local copy of the script has to be modified accordingly 
%% ..or see DTIprep-Gui -->scripts-button
%% =======================================================================
fi1=which('DTIscript_exportToHPC_makeBatch_21-11-2024.m');
...
%% #################################################
% pipeline
% DWI-analysis part-2 (MULTISHELL): script to transfer files to HPC and create starterscript(s) on local HPC-folder 
% this is a modified local copy of 'DTIscript_exportToHPC_makeBatch_21-11-2024.m'
% ..or see DTIprep-Gui -->scripts-button
...
%% -------------[HPC-TARGET-PATH]-------------------------------------------------------------
paHPCstudy_asW     = 'X:\Imaging\Paul_DTI\2025_NMDR_LGI1'  ;% HPC-TARGET-path as seen from WINDOWS (asW)

%% ------------[DTI_export4mrtrix-path]------------------------------------------------------------
% [painDat]: WINDOWS-folder withINPUT-data for DTI-processing ; form ["studypath"+"DTI_export4mrtrix"+"dat"]
painDat  = fullfile( 'H:\Daten-2\Imaging\AG_Pruess\2025_NMDR_LGI1'    ,...
    'DTI_export4mrtrix');

%% ------------[jobname]-------------------------------------
bash.jobName       = 'lg1'          ; % arbitrary name of jobname in bashfile (no space, no sepcial characters,short)
% --------------------------------------------------------------------------

%______ animals to export to HPC-CLUSTER and USE in batch(s) (use "all" or a numeric vector) ____________
p.numBatchfiles    = 4        ; %number of batchfiles, each batchfile process a subset of animals (default: 1 )
p.animals          = 'all'    ;%animals to copy to HPC: 'all': all inimals from "painDat"-dir
% p.animals        = [1:2]    ;%animals to copy  to HPC: 'all': all inimals from "painDat"-dir, or numeric array ; examples: [1 2 4], [4] or [4,7:10] 

% ----------------[other batch-parameter]--------
bash.jobTime       = '48:00:00'   ; % max-limit on total HPC run time: example: "22:00:00" for 22hours; NOTE: max-allowd time is restricted to 48h ("48:00:00")  ...The 7 days ("7-00") option does not work anymore on our HPC  
bash.jobCPUs       = 16          ; % number of CPUs (cores) per task'; number of CPUs (50 seems to be ok, 120 seems to be a bit better but maybe you must wait to obtain the ressources)
p.copydirs         = 1            ; % copy the data to HPC: [1] yes, [0] no
p.overwriteDirs    = 0            ; % force to overwrite data on HPC: [1]yes, [0] no, if folder exist do not copy data
...
% ----the following applies only if "p.transferBatch" is "1"
p.transferBatch    = 4;                                                               ; % [0,1]transfer batchfile to HPC; [0]no just show batch(copy/nPaste); [1]yes, write batch(sh-file) to HPC-cluster
p.HPC_hostname     = 's-sc-frontend2.charite.de'                                      ; % HPC-hostname:  old HOSTNAME: 's-sc-test4.charite.de
p.makeExecutable   = 1;                                                               ; % make shellscript executable ('chmod' starter shellscript)
p.condaEnvironment = '/sc-projects/sc-proj-agtiermrt/Daten-2/condaEnvs/dtistuff'      ; % name of the conda-environment (containing mrtrix,fsl,ants); otherwise leave empty ('')
p.start_shfile     = 'run_mrtrix.sh'                                                  ; % shellscript linked in the HPC-batchfile: this file is executed from the starting shellscript (previously:  'rat_exvivo_complete_7expmri.sh' or 'mouse_dti_complete_7texpmri.sh')
