...
%%   select similar
% ===============================================
px=uicontrol(hp,'Style','pushbutton','units','normalized',        ...
    'Position',[.65 .05 .1 .02],'tag','findsimilar');
set(px,'string','select similar','callback',{@findsimilar},'fontsize',fontsize);
set(px,'position',[0.195 0.4938 0.1 0.22]);
set(px,'tooltipstring','select similar items','foregroundcolor','b');
%% ==================[msgbox]=============================
...
delete(findobj(hf,'tag','hsim'));
panpos=[100 200 200 400];
hpa=uipanel('title','Select similar items','units','pixels',...
    'position',    panpos,'tag','hsim');

pos=[0 .9 .5 .05];
spaceUp=0.1;
stp=0.042;
posy=1-((1:length(hd))*stp-stp)-spaceUp;
for i=1:length(hd)
   hg=uicontrol(hpa,'style','checkbox','tag','sim_rad','userdata',hd{i},'value',0,'units','norm') ;
   pos2=pos;
   pos2(2)=posy(i);
   set(hg,'position',pos2)
   set(hg,'string',[ hd{i}],'fontsize',7);
end

%% ===========[txt]====================================
hb=uicontrol(hpa,'style','text','units','norm','string','similar regarding: ');
set(hb,'position',[0 .94 1 .035],'HorizontalAlignment','left','fontweight','bold','foregroundcolor','b');

%% ===========[select]====================================
hb=uicontrol(hpa,'style','pushbutton','units','norm');
set(hb,'string','select');
set(hb,'position',[ 0.4820    0.7    0.3    0.0500],...
    'tag','hsim_sel','callback', {@findsim_CB,'select'});
%% ===========[DE-select]====================================
hb=uicontrol(hpa,'style','pushbutton','units','norm');
set(hb,'string','deselect');
set(hb,'position',[ 0.4820    0.65    0.3    0.0500],...
    'tag','hsim_desel','callback', {@findsim_CB,'deselect'});
%% ===========[DE-select-all]====================================
hb=uicontrol(hpa,'style','pushbutton','units','norm');
set(hb,'string','deselect all');
set(hb,'position',[ 0.4820    0.6    0.3    0.0500],...
    'tag','hsim_sel','callback', {@findsim_CB,'deselectAll'});
%% ===========[help]====================================
hb=uicontrol(hpa,'style','pushbutton','units','norm');
set(hb,'string','help');
set(hb,'position',[ 0.4820    0.45    0.3    0.0500],...
    'tag','hsim_hlp','callback', {@findsim_CB,'help'});
%% =============[close]==================================
hb=uicontrol(hpa,'style','pushbutton','units','norm','string','close');
set(hb,'position',[  0.4820    0.4    0.3    0.0500],...
    'tag','hsim_close','callback', {@findsim_CB,'close'});
%% ===============================================
%% ==============pan-tool=================================
...
hp=uicontrol('style','pushbutton','units','pixels','tag', 'hsim_pbmovepan');
...
set(hp,'position',[panpos(1)+panpos(3)-17 panpos(2)+panpos(4)-17 15 15 ])
je = findjobj(hp); % hTable is the handle to the uitable object
set(je,'MouseDraggedCallback',{@hsim_motio,hpa}  );
% set(hp,'units','pixels');
% pos=get(hp,'position');
...
if strcmp(task,'close')
    delete(findobj(hf,'tag','hsim'));
     delete(findobj(hf,'tag','hsim_pbmovepan'));
elseif strcmp(task,'select') || strcmp(task,'deselect') || strcmp(task,'deselectAll')
    %% ===============================================
    if strcmp(task,'deselectAll')
        selectthis([],0);
        return
    end
    ...
    hd=u.header;
    d=u.raw;
    hp=findobj(hf,'tag','hsim');
    hs=findobj(hp,'tag','sim_rad');
    sel=get(hs(find(cell2mat(get(hs,'value'))==1)), 'userdata');
    if isempty(sel); return; end
    
    ix=find(ismember(hd,sel)) ; %selected columns
    
    %previously selected table-rows
    prevsel=find(u.sel);
    tp=d(prevsel,:);
    ...
    for i=1:size(tp,1)
        ...
        z=[];
        for j=1:size(given,2)
            z(:,j) =strcmp(serch(:,j),given(j));
        end
        iv=find(prod(z,2));
        id=[id; iv(:)];
    ...
    if strcmp(task,'select')
        ...
        ub.prevsel=prevsel;
        set(hp,'userdata',ub);
    elseif strcmp(task,'deselect') %deselect
        hp=findobj(hf,'tag','hsim');
        ub=get(hp,'userdata');
%         try
            selectthis([],0);
            set(lb1,'value', ub.prevsel);
            selectthis([],3);
...
%         end
 ...
 elseif strcmp(task,'help') 
     ...
     g={' #lk HELP: select similar items'
         '1) select files-of-interest from one animal '
         '     for example select: "T2map_MSME_CRP" & "DTI_EPI_seg_30dir_sat" '
         '2) set relevant ckeckboxes to [1]'
         '     for example set checkboxes of "protocol" and "proc#"-number'
         '3)  hit [select]-button to select the fiels across all animals' 
         ''
         };
     uhelp(g,0,'name','HELP: select similar items');
%% ===============================================
...
set(hp1,'position',[.3 .6 .15 .1],'string','add item>',...
    'backgroundcolor',[1.0000    0.9490    0.8667]);
set(hp1,'tooltipstring',[...
    '<html><b>add item</b><br>' ...
    'parse selected items from left listbox to right edit box'...
    ''],'fontsize',7);
%% ===============================================
% add constraint
hp1=uicontrol('parent',hpa,'style','pushbutton','units','normalized','tag','hfm_getfromLB','string','>',...
    'callback',{@findnested_callback,'selectFromLB_add'});
set(hp1,'position',[.3 .3 .15 .1],'string','&& add item>',...
    'backgroundcolor',[1.0000    0.9490    0.8667]);
set(hp1,'tooltipstring',[...
    '<html><b>add another item</b><br>' ...
    'add another item (B) which constrains selection (i.e.: A & B must be true) '...
    ''],'fontsize',7);
%% ===============================================
...
elseif strcmp(task,'selectFromLB')
    ...
    s3=[s2; {b0}];
    set(he,'string',s3);
 elseif strcmp(task,'selectFromLB_add')
    %% ===============================================
...
elseif strcmp(task,'select') || strcmp(task,'deselect')
    ...
    for j=1:size(s,1)
        q=s{j};
        for i=1:length(dh)
            q= regexprep(q,['\$' dh{i} ':'],[ '~cellfun(@isempty, regexpi(d(:,' num2str(i) '),''' '']);
        end
        q=regexprep(q,' && ', ''')) & ');
        q=[q '''))'];
        q=['iv=' q ';'];
    ...
    %     ~cellfun(@isempty,regexp(d(:,5),'4|5'))
    % ~cellfun(@isempty, regexpi(d(:,5),'4')) & ~cellfun(@isempty, regexpi(d(:,6),'1'))
    ...
    if 0
        ...
        for i=1:length(v.hd)
            ...
            t= regexprep(s,['\$' dh{i} ':'],[v.hd{i} '$split$'])
            
            t= regexprep(s,['\$' dh{i} ':'],[ 'regexpi(d(:,' num2str(i) '),' ''])
        end
        
        
        strcmp(d(:,5),'4') & strcmp(d(:,6),'1')
        
        %% ===============================================
        s=cellstr(get(he,'string'));
        for i=1:length(v.hd)
            s= regexprep(s,['\$' v.hd{i} ':'],[v.hd{i} '$split$']);
        end
        
        [dv]={};
        for i=1:size(s,1)
            [q1 ]= strsplit(s{i},'$split$');
            %        if length(q1)==2
            dv(end+1,:)=q1;
            %        end
        end
