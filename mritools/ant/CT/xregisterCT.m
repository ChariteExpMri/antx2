...
%               at once with different aimed interp-orders (default)
...
% paramfile : Elastix parameter file for registration:
%             select either a rigid or an affine transform:
%              - (1) for RIGID  transform use: antx\mritools\elastix\paramfiles\CT_rigid.txt
%              - (2) for AFFINE transform use: antx\mritools\elastix\paramfiles\CT_affine.txt
% method  : registration approach: 1 or 2
%               The idea of the methods is extract a (binary) brain volume from the CT-image which is than registered
%               to the brain-volume of the t2w-image (aka the thresholded 'ix_AVGT.nii')
%                [1] otsu+morphological operations 
%                [2] otsu+morphological operations+watershed segmentation
%                method 2 seems to work better than 1
% dilate   : dilate brain by this vox-size to account for brain-csf-skull-distance (a number) 
%             if  dilate=0:   this does not dilate/erode the CTbrainMask
%             if  dilate>0:   this dilates the CTbrainMask --> this shrinks the CT image in ALLEN-SPACE
%             if  dilate<0:   this erodes the CTbrainMask --> this blows up the CT image in ALLEN-SPACE
%          - dilate=2 worked good with this data
% 
% bonebrainvolume: total volume [qmm] of bone/skull and within-skull-structures (brain/CSF)
%            - only used in method-2!
%            - default 1200 qmm for mouse, as an rough estimate for the total volume of 
%              all bones (skull+spinal bones) and the volume of within bone structures (brain+csf+..)
%            - this is used to obtain the proper cluster from otsu-segmentation
...
% #wg EXAMPLES
%
%% EXAMPLE-1: CALCULATE FORWARD & BACKWARD transformation (has to be done once). 
% Use Affine trafo to bring 'CT.nii' to 't2.nii'-Space, than apply transformation to 'hSPECT.nii' 
% to bring it in 't2.nii' space, too. The transformed image files have the prefix 'R'. Interpolation
% is set to auto (..autodetect interpolation). Direction is set to 1, because this forward direction 
% is mandatory for initial calculation of the transformation. 
% Here the AFFINE trafo ist used. For RIGID body trafo select 'CT_rigid.txt'.
% method-2 is used which uses a watershed segementation to extract the "brain" from the CT-image
% dilate-2 is used which dilates the CT-brain by 2 voxels to account for skull/CSF/BrainBoundary-distance
% dimspace is empty .... the output has the dimensions of the reference image
% 
% z=[];                                                                                                                                                                                                                         
% z.CTimg     = { 'CT.nii' };                                            % %  << select the CT-image                                                                                                                            
% z.APPimg    = { 'hSPECT.nii' };                                        % %  << select other images (0,1 or n-images) for which the transformation should be applied                                                           
...
% z.paramfile = 'o:\antx\mritools\elastix\paramfiles\CT_affine.txt';     % % Elastix parameter file for registration (use either: "CT_rigid.txt" or "CT_affine.txt")                                                            
% z.method    = [2];                                                     % % method to obtain the CT-"brain", current methods: 1 or 2                                                                                           
% z.dilate    = [2];                                                     % % dilate brain by this vox-size to account for brain-csf-skull-distance                                                                              
% z.bonebrainvolume =[1200]                                              % % bone brain volume
...
% xregisterCT(1,z);  
% ================================================================================================
%% ONCE THE FORWARD & BACKWARD transformation is calculated you can transform other images
%% EXAMPLE-2 transform another image to t2.nii-space (FORWARD)
% here 'hSPECT.nii'  ist transformed to t2.nii-space
% z=[];                                                                                                                                                                                                                         
...
% xregisterCT(1,z);  
% 
%% EXAMPLE-3 transform another image to ct.nii-space (BACKWARD)
% here 't2.nii'  ist transformed to CT-space
% z=[];                                                                                                                                                                                                                         
% z.CTimg     = { '' };                  % %  << select the CT-image                                                                                                                            
...
function xregisterCT(showgui,x,pa)
...
if ~isfield(x,'paramfile')
    ...
    x.paramfile=which('CT_rigid.txt');
end
...
p={...
    ...
    'paramfile'       x.paramfile      'Elastix parameter file for registration (use either: "CT_rigid.txt" or "CT_affine.txt")'  {@getparamfile}
    'method'            2              'method to obtain the CT-"brain", current methods: 1 or 2  '    ''
   
    'dilate'            2              'dilate brain by this vox-size to account for brain-csf-skull-distance'  ''
    'bonebrainvolume'   1200           'total volume [qmm] of bone/skull and within-skull-structures (brain/CSF) ' ''
...
if ~isempty(z.CTimg)
    ...
    if z.method == 1
        resm=CTgetbrain_approach1(z, pa);
    elseif z.method == 2
        resm=CTgetbrain_approach2(z, pa);
    else
       error(' chose method 1 or 2')
    end
        
    
% % % % %     
% % % % % % ############################# START APPROACHES HERE #######################################################
% % % % % 
% % % % %     
% % % % %     %============================================================================================================================================
% % % % %     %% getting CT image
...
% % % % %     % ############################# END APPROACHES HERE #######################################################
    ...
    %%  elastix forward
    %============================================================================================================================================
    fprintf(['..running Elastix forward with Parameterfiles: \n']);
    ...
    disp(char(pf)); %PARAMETERFILES USED
