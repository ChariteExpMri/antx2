...
%     [0] normal seach
%     [1] reverse search
%   -the reverse search direction will not change the list1-list2 assignment/direction
%   
...
function [o]=gui_assign(list1,list2,varargin);
warning off
o=[];
...
if exist('list1')==0
    ...
    o=gui_assign(list1,list2,'ndirs','','matchtype',1,'wait',0);
    return
end
...
%     o=gui_assign(list1,list2);
    %o=gui_assign(list1,list2,'ndirs',2,'matchtype',1);
    o=gui_assign(list1,list2,'ndirs',2,'matchtype',1,'wait',0);
%     list2=['rrT2_ra,_265_20G_1_G_d195' ; list2; 'T2_266_20G_1_G_d195' ; ];
...
[s hs]=findkeys2(keys,list,'show',0);
% ==============================================
...
if 1
   ...
   % bgcol=round(cbrewer('qual','Pastel1',3).*255);
   bgcol=[0.7569    0.8667    0.7765
          0.9529    0.8706    0.7333];
    bgcol=round(bgcol*255);
    bgcol=repmat(bgcol,[ size(mdirsLB,1) 1]);
...
set(ht,'string',[p.lab_show_nNout],'backgroundcolor','w','fontsize',8);
set(ht,'position',[0.45643 -0.0082038 0.08 0.03]);
ht.HorizontalAlignment='right';
%===================EDIT===========================
hb=uicontrol('style','edit','units','norm','tag','ed_ndirs');
set(hb,'string',p.ndirs,'backgroundcolor','w');
set(hb,'position',[0.5392 0.002 0.03 0.03]);
...
set(hb,'tooltipstring',['<html><b>display selected input-output addignments</b>'...
    '<br> you have to select an output-dir']);
% ==============[clear]=================================
hb=uicontrol('style','pushbutton','units','norm','tag','pb_clear');
set(hb,'string','deleselct all','backgroundcolor','w');
set(hb,'callback',@call_clear);
set(hb,'position',[0.19 0.002 0.06 0.025]);
set(hb,'tooltipstring',['<html><b>deselect all (uncheck all checkboxes)</b>']);
% ==============OK=================================
...
set(hb,'backgroundcolor',[1.0000    0.9686    0.9216]);
set(hb,'tooltipstring',['<html><b>assignments are made--> proceed</b>']);
...
u=get(gcf,'userdata');
hb=findobj(gcf,'tag','pb_showmatch');
hb_string=get(hb,'string');
hb_bgcol=get(hb,'backgroundcolor');

set(hb,'string','..please wait!');
...
%% ========[ common string across all LCS ]=======================================
tb=u.tb;
len_commonLCS=max(cell2mat(cellfun(@(a){[ length(a)]}, tb(:,6))));
mtab=cellfun(@(a){[ a  repmat(' ',[ 1 len_commonLCS-length(a) ])  ]}, tb(:,6));
mtab=cellfun(@(a){[ double(a) ]}, mtab);
mtab=cell2mat(mtab);
...
for i=1:size(bf,1)
    ...
    if isempty(is)
        ...
        if 1
            ...
            %s1=u.s0{i,2}{is,4}
            %s2=u.s0{i,2}{is,5}
            try; lcs(commonString)=[]; end
            
            
            r{4}=['<b>' regexprep(r{4},lcs,[ '<b><u><font color=fuchsia>' lcs  '</font></b></u>']) ];
            r(2)=regexprep(r(2),lcs,[ '<b><u><font color=fuchsia>' lcs  '</font></b></u>']);
            
%             %w=r{4};
%             w=u.s0{i,2}{is,5}; %q(ix);
%             w2={};
%             for j=1:length(w)
%                 w2{j,1}=w(j) ;
%             end;
...
set(hb,'string',hb_string);
set(hb,'backgroundcolor',hb_bgcol);
...
try; jtable.changeSelection(irow+50,0, false, false); end
try; jtable.changeSelection(irow-1,0, false, false); end
...
for i=1:length(is)
    ...
    tb{is(i),3}=['<html><p style="background-color:rgb(' colstr ');">' tb{is(i),3}] ;
    %    if i==1
    %       tb(is(i),2)={logical(1)} ;
    %    end
end
...
for i=1:length(mdirs)
    ...
    for j=1:size(tg,1)
        lcs=tg{j,6};
        try; lcs(commonString)=[]; end
        tg{j,3}=['<html>' regexprep(tg{j,3} ,lcs,['<b><font color=fuchsia>'   lcs  '</b><font/>' ])];
        tg{j,1}=regexprep(tg{j,1},lcs,['<b><font color=fuchsia>'   lcs  '</b><font/>' ]);
    end
...
t1=regexprep( tb(:,1), '<.*?>', '' );
nums=cell2mat(cellfun(@(a){[ str2num(a)]}, regexprep(t1,{'].*','['},{''})));
nums_uni=unique(nums);
bgcol=u.bgcolTable;
bgcolvec=repmat([1:size(bgcol,1)],[1 length(nums_uni) ]);
