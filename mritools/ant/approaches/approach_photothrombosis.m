% #ok approach_photothrombosis.m: 
% #b DETAILS: 
% Photothrombotic Ischemia with large deformations
% #r ____________________________________________________
% #r The "masklesion.nii" file is mandatory here!
% The "masklesion.nii" is the lesion mask of the  respective animal
% Properties of "masklesion.nii": 
%   -binary mask, value of "1" is a voxel with lesion
%   -must be stored in the local animal-dir/file name must be "masklesion.nii"
%   -when drawing the lesion, draw it to the assumed brain-boundary  
% #r ____________________________________________________
% 
...
% #r               -"masklesion.nii" is used to preserve the brain-boundaries
...
function approach_photothrombosis(s)
...
%% % ===============================================  
%%  [2+3]  make MOUSE IMAGE FOR REGISTRATION
%% % ===============================================  
...
if exist(fullfile(s.pa,'masklesion.nii'))==2
    ...
    val=a(e==1);    val(val==0)=[]; val=median(val); if isempty(val); val=0.3; end
    a(e==1)=val;
    
    val=b(e==1);    val(val==0)=[]; val=median(val); if isempty(val); val=0.3; end
    b(e==1)=val;
    c=a*10000+b*20000+1000;
    c=round(normalize01(c).*500);
