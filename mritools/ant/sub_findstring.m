...
function searchfun(e,e2,colindex)

pbexitsearch;
hfig2=gcf;%findobj(0,'tag','selector');

ut=findobj(hfig2,'type','uitable');
uv.ht=ut;
uv.header=ut.ColumnName;
if exist('colindex')==0
    uv.searchcolumn=1;
else
    uv.searchcolumn=colindex;
end
...
set(hpd,'position',[pos(1) pos(2)+hig .2 pos(4)  ],'string',uv.header,'value',uv.searchcolumn);
...
set(he,'backgroundcolor',[1 1 0],'callback',@btn_exactmatch);
...
function btn_exactmatch(e,e2)
if e.Value==1
    set(e,'backgroundcolor',[1 1 0]);
else
     set(e,'backgroundcolor',repmat(.94,[1 3]));
end
...
d=uv.ht.Data;
c = regexprep( d(:,icol), {'<.*?>','&nbsp;','\s+$'}, '' );

hexact= findobj(gcf,'tag','toggleexactsearch');
if get(hexact,'value')==1
    is=regexpi2(c,['^' str2search '$']);
    if isempty(is)
        is=regexpi2(c,['^' regexprep(str2search,[filesep filesep],[filesep filesep filesep]) '$'] );
        %is=find(strcmp(c, str2search));
    end
else
    is=regexpi2(c,str2search);
    if isempty(is)
...
for i=1:length(is)
    ...
    jUIScrollPane = findjobj(uv.ht);
    jt = jUIScrollPane.getViewport.getView;
    jt.changeSelection(row-1,col-1, false, false);
end
%% ===============================================
