...
%-------------------
if isfield(pp,'fs'); sx.label_fs=pp.fs; end
prepimage(sx);
prepdata();
...
% ===============================================
if 0
    dtistat('plot','atlas',atlas,'label',1,'value',1)
     dtistat('plot','atlas',atlas,'thr',3) 
     dtistat('plot','atlas',atlas,'thr',2,'p',0.01) 
     dtistat('plot','atlas',atlas,'thr',3)
     dtistat('plot','atlas',atlas,'thr',2,'p',0.01,'label',1)
     dtistat('plot','atlas',atlas,'cmap',3)
end


%% =========work on batch-parameter======================================
if isfield(pp,'thr') || isfield(pp,'p');
    if isfield(pp,'thr');
        set( findobj(findobj(0,'tag','dtifig1'),'tag','threshtype'), 'value',pp.thr);
        set_threshtype();
    end
    if isfield(pp,'p');
        set( findobj(findobj(0,'tag','dtifig1'),'tag','thresh'), 'string',num2str(pp.p));
        set_thresh();
...
if isfield(pp,'value'); 
    set( findobj(findobj(0,'tag','dtifig1'),'tag','show_statvalue'), 'value',pp.value);
    show_statvalue();
end
if isfield(pp,'label'); 
    set( findobj(findobj(0,'tag','dtifig1'),'tag','set_showlabel'), 'value',pp.label);
    set_showlabel();
end
if isfield(pp,'cmap'); 
...
delete(findobj(hf,'tag','tx_nosurvive'));
if isempty(s)
   % addNote(hf,'note','close') ;
...
set(findobj(hf,'type','image'),'visible','on');
...
warning off;
 s.doc='add';
% s.doc='new';
if isfield(p,'pptfile') && ~isempty(p.pptfile)
    pptfile=p.pptfile;
else
    return
end
...
p.hideControls=1;
hf=findobj(0,'tag','dtifig1');
hc=get(findobj(hf,'tag','contrast'));
us=get(hf,'userdata');
pw=us.us.pw(str2num(hc.String{hc.Value}));  %contrast info
fd=us.us.con.files{1};
ix=min([min(strfind(fd,[filesep 'dat' filesep])) min(strfind(fd,[filesep 'data' filesep]))]);
[~, studyname]=fileparts(fileparts(fd(1:ix)));
