...
% <b> script to make summary of voxelwise statistic FOR SEVERAL IMAGES and SEVERAL GROUPS   </b>  
% <font color=fuchsia><u>RUN THIS AFTER CALCULATION OF VOXELWISE STATISTIC FOR SEVERAL IMAGES AND SEVERAL GROUPS!:</u></font>
%
...
spmdirMain='F:\data5\eranet_round2_statistic\voxstat\voxstat1';
...
PPTFILEprefix  = 'res_voxw_10mar22';              % PowerPoint - fileNamePrefix

 % [4] output-folder of excel-files and thresholded volumes
outdirMain         = fullfile(spmdirMain,'res_voxw_10mar22'); % output-folder for Excelfiles & volumes
...
PPTFILE1=fullfile(outdirMain,[PPTFILEprefix '_CLUSTCORECTED' '.pptx'   ]);
PPTFILE2=fullfile(outdirMain,[PPTFILEprefix '_FWE' '.pptx'   ]);

makePPTslide('doc','new','file',PPTFILE1,'fs',70,'text',{PPTFILEprefix 'cluster-Corrected'},'bgcol',[1 1 .5 ]);
makePPTslide('doc','new','file',PPTFILE2,'fs',70,'text',{PPTFILEprefix 'FWE-corrected'},'bgcol',[1 1 .5 ]);
...
for g=1:length(spmdirs) % SPM_folders
    ...
    % ========PREPARE PPT
    [~, imageName ext]=fileparts(SPM.xY.P{1});
    msg={[regexprep(xSPM.title,{'<|>'},' vs ') '  [con]' ], [  imageName ext '  [image]' ],[ spmdirsShort{g} '  [dir]'] }';
    makePPTslide('doc','add','file',PPTFILE1,'fs',50,'text',msg,'bgcol',[0.8039    0.8784    0.9686 ]);
    makePPTslide('doc','add','file',PPTFILE2,'fs',50,'text',msg,'bgcol',[0.8039    0.8784    0.9686]);
    ...
    for con=1:2
        if mod(con,2)==1 % alterate PPT-backgroundcolor for each contrast
            ...
            DOC='add';  % for each SPM-DIR, if contrast is 1--> create "new" PPTfile
        else
        ...
        if 0 % 
            ...
            xstat('report',PPTFILE1,struct('doc',DOC,'con',con,'bgcol',bgcol  )); % save stat. table in PPT
            xstat('report',PPTFILE1,struct('show','volume','doc','add','con',con,'bgcol',bgcol  )); % save volume in PPT
        ...
        xstat('report',PPTFILE1,struct('doc','add','con',con,'bgcol',bgcol    )); % save stat. table in PPT
        xstat('report',PPTFILE1,struct('show','volume','doc','add','con',con,'bgcol',bgcol  ));% save volume in PPT
        xstat('savevolume',outdir); % save tresholded volume
        xstat('export',outdir);     % save stat. table as excelfile
        ...
        xstat('report',PPTFILE2,struct('doc','add','con',con,'bgcol',bgcol    )); % save stat. table in PPT
        xstat('report',PPTFILE2,struct('show','volume','doc','add','con',con,'bgcol',bgcol  )); % save volume in PPT
        xstat('savevolume',outdir); % save tresholded volume
        xstat('export',outdir);     % save stat. table as excelfile
