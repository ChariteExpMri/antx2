...
for i=1:length(images) % loop over each image
    ...
    z.inputimage     = imageName                                            % % name of the NIFTI-image to analyze ("data_dir" has to bee defined before using the icon)
    ...
    z.output_dir     = fullfile(pastudy,outdir,outdirSub) ;                 % % path of the output-directory (SPM-statistic with SPM.mat and images)
