function varargout=elastix2(movingImage,fixedImage,outputDir,paramFile,elxpath, varargin)
...
% disp('bla')
% disp('F:\data3\histo2\bart\elastix2 ####paul')
% pael= 'F:\data3\histo2\bart\elastix2'  
pael=elxpath;
%----------------------------------------------------------------------
% *** Handle default options ***

%Confirm that the elastix binary is present
% [s,elastix_version] = system('F:\data3\histo2\bart\elastix2\elastix --version');
[s,elastix_version] = system([ pael filesep 'elastix --version' ]);
...
if nargin==1 & ischar(movingImage)
    if regexp(movingImage,'^\w')
       % [s,msg]=system(['F:\data3\histo2\bart\elastix2\elastix --',movingImage]);
        [s,msg]=system([  pael filesep 'elastix --',movingImage]);
    end
    fprintf(msg)
    return
end
...
mhd_write(movingImage,fullfile(outputDir,movingFname));
...
%     end
    ...
    mhd_write(fixedImage,fullfile(outputDir,targetFname));
end
...
% disp('F:\data3\histo2\bart\elastix2 ####paul')
CMD=sprintf('%selastix -f %s.mhd -m %s.mhd -out %s ',...
            [pael filesep],...
...
if status %Things failed. Oh dear. 
    ...
    if nargout>1
        ...
        if isempty(d)
            ...
            for ii=1:length(d)
%                 disp('critical');
%                 disp([' outputPaul: ' outputDir,filesep,d(ii).name]);
