...
[s,elastix_version] = system([elastixFP ' --version'])  ;%'elastix --version');
...
if nargin==1 & ischar(movingImage)
    if regexp(movingImage,'^\w')
        ...
        [s,msg]=system([ elastixFP ' --',movingImage]);
    end
    fprintf(msg)
    return
end
...
%             outputDir);
CMD2=sprintf([' -f %s.mhd -m %s.mhd -out %s '],...
            fullfile(outputDir,targetFname),...
            fullfile(outputDir,movingFname),...
            outputDir);
CMD =[ elastixFP  CMD2 ]        
