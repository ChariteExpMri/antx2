...
if ispc
    transformixFP = which('transformix.exe');
elseif ismac
    ela=elastix4mac;
    transformixFP=ela.E;
else
...
%             outputDir,filesep,paramFname);
        CMD2=sprintf(' -in %s%s%s -out %s -tp %s%s%s',...
                    outputDir,filesep,movingFname,...
                    outputDir,...
                    outputDir,filesep,paramFname);
         CMD=[transformixFP CMD2]   ;    
...
if nargin>1
    ...
    elseif size(movingImage,2)>3 %It's an image
        ...
        CMD2 = sprintf(' -in %s.mhd ',movingFname);
        CMD=[transformixFP CMD2]   ;
    elseif size(movingImage,2)==2 | size(movingImage,2)==3 %It's sparse points
        ...
        CMD2 = sprintf(' -def %s ',movingFname);
        CMD=[transformixFP CMD2]   ;
    else
